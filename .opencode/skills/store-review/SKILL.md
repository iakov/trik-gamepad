...
| F14 | **Source reproducibility** | Recommended | `uv run python scripts/check_reproducibility.py` — builds twice, compares APK hashes | [Reproducibility](https://f-droid.org/docs/Reproducible_Builds/) |
| F15 | **`assembleRelease` without keystore** | Yes (policy, DECISIONS.md 2026-09-23) | `scripts/check_reproducibility.py` implicitly verifies (moves keystore aside) | [Reproducibility](https://f-droid.org/docs/Reproducible_Builds/) |
| F16 | **Zero build timestamps** | Yes (policy) | `grep 'build.time.zero' gradle.properties` → `true` present | [Reproducible Builds](https://f-droid.org/docs/Reproducible_Builds/) |
| F17 | **Wi-Fi feature declared** | Yes | `grep 'uses-feature.*wifi' app/src/main/AndroidManifest.xml` → `android.hardware.wifi` `required="true"` | [Manifest scanner](https://f-droid.org/docs/Building_Apps/) |
