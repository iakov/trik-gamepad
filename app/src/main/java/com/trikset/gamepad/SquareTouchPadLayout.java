...
public class SquareTouchPadLayout extends RelativeLayout {
  ...
  private void init() {
    ...
    setBackgroundDrawable(
        ResourcesCompat.getDrawable(
            getResources(), R.drawable.oxygen_actions_transform_move_icon, null));
  }
  ...
  final class TouchPadListener implements OnTouchListener {
    ...
    /**
     * Handles touch events; the default case deliberately comes first — it logs the unknown action
     * and returns, so all real action codes are handled below without a trailing default.
     *
    ...
    public boolean onTouch(@NonNull final View v, @NonNull final MotionEvent event) {
      ...
      switch (event.getAction()) {
        ...
        case MotionEvent.ACTION_DOWN:
          ...
          v.performClick();
          // fall through: DOWN starts tracking the pointer, MOVE continues it
        case MotionEvent.ACTION_MOVE:
          setAbsXY(
              Math.max(0, Math.min(event.getX(), mMaxX)),
              Math.max(0, Math.min(event.getY(), mMaxY)));
