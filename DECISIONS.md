...
### [2026-09-23] F-Droid reproducible builds are mandatory for every release

- **Type:** policy / cross-cutting (every release MUST be reproducible-build compatible so F-Droid can ship the developer-signed APK instead of re-signing with its own key).
- **Problem:** F-Droid normally rebuilds from source and replaces the developer signature with the F-Droid key ("signed by F-Droid" in the app). Users then have to trust F-Droid's key infrastructure rather than the original developer. If the build is **reproducible** (F-Droid's APK is byte-identical to the developer's APK, ignoring the signature block), F-Droid ships the developer's original APK — giving users a trust chain back to the maintainer.
- **Alternatives considered:** (a) reproducible builds (chosen — user trust, standard for reputable F-Droid apps); (b) accept F-Droid re-signing (rejected — loses the developer trust chain); (c) skip F-Droid entirely (rejected — it is a major free-software distribution channel).
- **Chosen solution:** make the release build deterministic: (1) `android.injected.build.time.zero = true` in `gradle.properties` (zero build timestamps); (2) build with the SAME toolchain F-Droid uses (JDK 21+); (3) verify reproducibility per release by building twice (fresh checkout, clean build) and diffing the unsigned APK hashes; (4) `assembleRelease` must succeed WITHOUT the local keystore (F-Droid has no access to it) — signing config uses `System.getenv("TRIK_GAMEPAD_STORE_PASSWORD")`/`TRIK_GAMEPAD_KEY_PASSWORD` with empty-string fallbacks, and the release build type only assigns the signing config when the keystore file exists.
- **Why:** the app is a good F-Droid citizen otherwise (Apache-2.0, zero anti-features, no non-free deps, minimal permissions, Wi-Fi feature now declared). Reproducibility is the remaining standard F-Droid quality bar; making it a hard requirement from the start avoids retrofitting later.
- **Out of scope / consequences:** REPRODUCIBLE vs reproducible: perfect bit-reproducibility may need more work (AAPT2 resource ordering, zipalign determinism). The v2.44 minimum is: zero timestamps + keystore-free build. Perfect reproducibility verification is a stretch goal to measure at release time. Every release's release-notes review MUST run the reproducibility check (build twice → compare hashes) and record the result.
