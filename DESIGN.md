...
| `custom <message>` | custom-message field confirmed in Robot settings (Network) | Opaque plain-text message to the robot, exposed to user programs (the robot emits a `custom` event). **Edge**: sent once when the message is confirmed — never re-sent by unrelated setting changes or the register sweep (the gamepad's `MainActivitySettingsController` sends only on the `customMessage` key change). Shipped in C31; the reference implementation accepts and logs it. |
