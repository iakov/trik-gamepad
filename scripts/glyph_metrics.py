...
def emit_kotlin(codepoints: dict[str, int], font_bytes: bytes, reference: str, out_path: str) -> None:
    ...
    if reference in codepoints:
        ...
        _ref_name, ref_vh, ref_mb, ref_ink = "1", 0.72, 0.0, 0.72
    ...
    lines = [
        ...
        " */",
        "object GlyphMetrics {",
        f"  const val LINE_BOX_EM = {line_box_em:.4f}f",
        ...
        f"  val REFERENCE_METRIC = GlyphMetric({ref_vh:.6f}f, {ref_mb:.6f}f, {ref_ink:.6f}f)",
        "",
        "  private val TABLE: Map<Int, GlyphMetric> = mapOf(",
    ]
    for cp in sorted(metrics):
        name, vh, mb, ink = metrics[cp]
        lines.append(f"      0x{cp:X} to GlyphMetric({vh:.6f}f, {mb:.6f}f, {ink:.6f}f), // {name}")
    lines += [
        "  )",
        "",
