...
def main() -> int:
    parser = argparse.ArgumentParser(description="List the failed job(s)/step(s) of a GitHub Actions run.")
    parser.add_argument("run_id", help="the gh run id (from `gh run list`)")
    parser.add_argument("--repo", default=default_repo(), help="owner/repo (default: origin remote)")
    ...
    for job in jobs:
        ...
        if failed or job.get("conclusion") == "failure":
            print(f"job={job['name']} conclusion={job.get('conclusion')} failed_steps={failed}")
    return 0
