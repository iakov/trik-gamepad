...
def main() -> int:
    parser = argparse.ArgumentParser(description="Print uiautomator dump views as readable rows.")
    ...
    for m in NODE_RE.finditer(text):
        ...
        haystack = f"{rid} {cls} {desc} {txt}"
        if args.filter and args.filter not in haystack:
            continue
        if not (rid or cls or desc or txt):
            continue
        found += 1
        print(f"id={rid or '-'} class={cls or '-'} desc={desc or '-'} text={txt or '-'} bounds={bounds or '-'}")
    if found == 0:
