...
def totals_mode(args: argparse.Namespace) -> int:
    ...
    for ctype, missed, covered in re.findall(r'<counter type="(\w+)" missed="(\d+)" covered="(\d+)"/>', xml):
    ...
    for ctype in ("INSTRUCTION", "LINE", "BRANCH", "COMPLEXITY", "METHOD", "CLASS"):
        ...
        print(f"  {ctype:12s} missed={missed:5d} covered={covered:5d} total={total:5d} ratio={ratio:.3f}")
    ...
    if args.class_filters:
        ...
        rows = []
        for m in re.finditer(r'<class name="([^"]+)" sourcefilename="[^"]+">(.*?)</class>', xml, re.DOTALL):
...
def lines_mode(args: argparse.Namespace) -> int:
    ...
    for cls in args.classes:
        ...
        for ln in nc:
            m = re.search(
                r'<(?:span|div|td) class="[a-z ]*nc[a-z ]*" id="L' + ln + r'">'
                r"(.*?)</(?:span|div|td)>",
                html,
                re.DOTALL,
            )
...
def main() -> int:
    ...
    totals.add_argument(
        "--class", dest="class_filters", action="append", default=[], help="substring filter on class name (repeatable)"
    )
