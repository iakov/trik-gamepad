    ...
    def loop() -> None:
        while True:
            try:
                ...
                return
            print(f"{ts()} < {addr[0]}:{addr[1]}: {data.decode('utf-8', 'replace').rstrip()}")

    return loop


def main() -> int:
    parser = argparse.ArgumentParser(description="Protocol-tracking gamepad client (logs every line both ways).")
    ...
    parser.add_argument("--batch", help="run ;-separated commands and exit (no quit needed)")
    ...
    print(
        "commands: pad N X Y | btn N [down|up] | wheel <angle> | keepalive <ms> | custom <message> | wait <ms> | quit"
    )
