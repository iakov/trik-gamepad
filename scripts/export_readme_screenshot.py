...
SRC = os.path.join(ROOT, "app", "build", "outputs", "screenshots", "hud_connected.png")
...
def main() -> int:
    ...
    if args.check:
        ...
        if os.path.getmtime(SRC) > os.path.getmtime(DST):
            print(
                f"STALE: source render is newer than the committed JPEG ({os.path.basename(DST)}) - re-run the export"
            )
            return 1
    ...
    print(f"size: {src_bytes / 1024:.0f} KiB -> {dst_bytes / 1024:.0f} KiB ({dst_bytes * 100 / src_bytes:.0f}%)")
    return 0
