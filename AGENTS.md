...
- **Verify current branch and its relationship to upstream/master:**
  `git branch --show-current; git remote -v; git merge-base HEAD upstream/master`.
  The `dev` branch is the permanent working branch on the fork; PR branches
  are created from `upstream/master`.

### Before commit

- Run `uv run pre-commit run --all-files` (or `uvx mdformat` on `.md`).
- `git commit --no-gpg-sign` (gpgsign=true locally, no interactive GPG).
- **NEVER use `git add -A` or `git add .` on a PR branch.** Always use
  explicit path lists: `git add app/src/.../File.kt`. The working tree
  always has `.omo/`, `.playwright-mcp/`, and other dev-only untracked
  files that `-A` would sweep in. If you accidentally stage them, use
  `git reset --hard <clean-hash>` to remove the commit entirely (not
  `git revert`, which pollutes history with a Revert commit).
...
- Fork-only workflow: work lives in personal fork (origin). PR branches are
  created from `upstream/master` and PRd to upstream `trikset/trik-gamepad`.
  The `dev` branch (fork) is the permanent development branch for agent docs
  and process improvements — NEVER PR `dev` to upstream.
- **Rebase onto upstream/master** before push: `git fetch upstream && git rebase upstream/master`.
  Confirm the base first (`git merge-base HEAD upstream/master`).
- **Review pending commits**: `git log --oneline --max-count=5` — verify each
  commit is self-contained and the message is Conventional Commits format.
- Branch: `feat/`/`fix/`/`docs/`/`style/`/`refactor/`/`chore/`. No direct master push.
- Squash-fix mistakes before push (`git reset --hard <clean-hash>`, NOT `git revert`).
  Verify `git diff --stat HEAD` shows only intended files before force-push.
...
- **PR metadata**: after every force-push, update the PR title + body via
  `gh api repos/X/Y/pulls/N -X PATCH` (NOT `gh pr edit`). First body line
  must match PR title verbatim. Verify with `gh pr view --json title,body`.
- **History rewrite safety**: before force-pushing a rewritten history, capture
  `git diff upstream/master..HEAD > .tmp/diff_before.txt`, do the rewrite,
  then compare byte-to-byte against the captured diff. Only push if identical.
...
- **Test assertions must trace the full call graph**: a bare assumption that stream
  error → VISIBLE misses that VideoRetryController.onStreamError → reload → LOADING
  hides it. The UI-thread final state after stream error is LOADING, not UNAVAILABLE.
- **Establish prior state before asserting transitions**: a test asserting GONE on
  a view whose default is already GONE passes with broken code. Always set up a
  differing prior state first so a regression would produce a visible delta.
- **Every release MUST be F-Droid reproducible-build compatible** (DECISIONS.md
  "[2026-09-23] F-Droid reproducible builds"). Before any release: (1) run
  `./gradlew clean assembleRelease` twice from fresh checkouts and compare the
  unsigned APK SHA-256 hashes — they must match; (2) `assembleRelease` must
  succeed WITHOUT the keystore (F-Droid has no access to it). Never add
  timestamps, build-time randomization, or embedded build paths that break
  reproducibility.
- **`assembleRelease` must never require the local keystore** — signing config
  reads passwords from `TRIK_GAMEPAD_STORE_PASSWORD`/`TRIK_GAMEPAD_KEY_PASSWORD`
  env vars (empty fallback) and only assigns the signing config when the
  keystore file exists. CI and F-Droid build unsigned.
...
- **Version bump MUST go through the version_manager script** — never hand-edit
  `version.properties` or the fastlane yml. `uv run python
  scripts/version_manager.py bump 45` (edits version.properties + syncs the
  fastlane/F-Droid metadata), then `version_manager.py check` must pass.
- Run the **release-notes skill** (`/.release-notes` or skill invocation): it
  produces `release-<tag>-tag-text.md` — line 1 = release title, rest = body.
  Review it; no secrets or internal paths.
- Create the signed annotated tag embedding the notes:
  `git tag -s -F release-v<major>.<minor>-tag-text.md v<major>.<minor> HEAD`
  (Abort if GPG signing hangs ≥10 min; locally only.)
- `git push upstream v<major>.<minor>` — CI builds the signed APK (`signed-release`
  job from the `signed_release` environment) then auto-publishes the GitHub
  release (`publish-release` job: title = tag head -1, body = tag rest, asset =
  `trik-gamepad-v<tag>-signed.apk`). No manual draft creation.
- After publish: refresh README hero screenshot if HUD changed.

## Priority tasks / status snapshot (when asked)

- **CI status**: `gh run list --repo iakov/trik-gamepad --branch dev --limit 3`
...
- **PR metadata sync**: after every force-push, update title + body via
  `gh api repos/X/Y/pulls/N -X PATCH`. First body line must match title.
  Verify with `gh pr view --json title,body` — `gh pr edit` can fail silently.
...
- **Store knowledge in the best-scoped doc**: AGENTS (rules) / DECISIONS
  ...
  (unfinished tasks).
- **Content boundaries**: DECISIONS.md = design decisions for all developers
  (committed, pushed upstream). MEMORY.md = agent memories (dev branch, never
  upstream). `.omo/` = session transient (local only, never committed). A
  session retrospective belongs in `.omo/notepads/` or MEMORY.md on `dev`,
  NEVER in the codebase as a standalone file.
...
uv run python scripts/check_reproducibility.py  # F-Droid reproducible build check (before release)
