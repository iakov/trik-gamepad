...
- **Scan the push for device identifiers before pushing** — the guardrail has
  been violated twice, so verification is now a push-prep step: `git grep -nE` the commits being pushed (`git diff origin/<branch>..HEAD`) for
...
- **Command hygiene — tee first, tail for status only**: every command that can fail MUST `tee` its output to `app/build/<task>.log` or `.tmp/<task>.log` before any `tail`/`head` truncation. Never bare-pipe `| tail` alone — the output is lost on truncation and a re-run for logs is a HARD FAIL (the first run produced the error; reading it from the log is mandatory). Pattern: `./gradlew … 2>&1 | tee app/build/<task>.log | tail -5`. On timeout or non-zero exit, read the saved log — do not re-run.
...
- **Re-read `.md` diffs after mdformat**: line-start `+`/`-`/`*` mid-paragraph get reflowed into lists — never start a wrapped line with a list character. Two more reflow traps from C26: a wrapped line starting with an operator (`+` for a continuation) becomes a list item, and a wrapped inline-code identifier inside a list item gets joined into one long line with a stray space — keep inline-code spans on a single line.
