...
VERSION_MAJOR/VERSION_MINOR live in `version.properties` (repo root) — the
single source of truth (never restated here; versions drift and are
discoverable from that file). `app/build.gradle` reads them to compute
`versionCode`/`versionName`. Bump ONLY via
`uv run python scripts/version_manager.py bump <minor>` (syncs the
fastlane/F-Droid metadata too); `version_manager.py check` verifies all
consumers agree.
...
Bump `VERSION_MINOR` per release; semantic 1.x is kept intentionally
...
1. Gates: green CI, 0 open PRs, 0 security alerts; `version_manager.py check` OK;
   `check_reproducibility.py` REPRODUCIBLE message.
1. Bump version ONLY via `scripts/version_manager.py bump <minor>` (syncs
   `version.properties` + fastlane yml); commit + PR.
1. Build `assembleRelease` locally (keystore present), smoke-test the APK.
1. Commit the APK to `_apk/` with a versioned name.
1. Run the `release-notes` skill → `release-v<tag>-tag-text.md` (line 1 =
   release title, rest = body). Review the tag text manually — no secrets.
1. Signed tag: `git tag -s -F release-v<tag>-tag-text.md v<tag> HEAD`;
   `git push upstream v<tag>`. CI `signed-release` (needs `signed_release`
   environment secrets) builds the signed APK, then `publish-release`
   auto-creates the GitHub release: title = tag `head -1`, body = tag rest,
   asset = `trik-gamepad-v<tag>-signed.apk`. **Publishing is automatic on
   signed-tag push + signed-APK build success — no manual drafts.**
...
**F-Droid & store compliance learnings (C40 aftermath):**

- **Fastlane is NOT required** — F-Droid accepts Triple-T (`app/src/main/play/`) or direct `.yml` metadata in fdroiddata. Only `short_description.txt` + `full_description.txt` are mandatory.
- **Amazon Appstore for Android discontinued** (Aug 20, 2025) — only Fire TV/Tablet remains. Our Amazon section rewritten accordingly.
- **Google Play G1 is targetSdk, not minSdk** — the Aug 2025 requirement is targetSdk ≥ 34, not minSdk.
- `uses-feature android.hardware.wifi` should be `required="true"` for our Wi-Fi-dependent app; location should be `required="false"`.
- **fdroiddata build metadata** can live as reference copy in our repo at `fastlane/metadata/com.trikset.gamepad2.yml` but is submitted to `fdroiddata.git` at submission time.
- **Tags only on upstream** — fork tags are useless garbage. Never tag before prompted.
- **UI/UX changes always update README + screenshots** — visual changes need a screenshot added to `docs/img/` and reflected in README.
- **Video timeout:** if no camera → 10s timeout stops retry loop, chip shows UNAVAILABLE, badge shown. `VideoRetryController` add `loadTimeoutMs` + `onTimeout` parameters.
- **Hardware button mapping** (A→gamepadButton1, B→gamepadButton2, etc.) documented in README for students.
- **`.omo/` cleanup pattern for PR branches:** `git reset --soft HEAD~1 && git rm --cached -r .omo/ && git commit` — necessary because `.omo/` exists in dev worktree and `git add -A` always picks it up.
- **PR diff check before push:** Always review `git diff upstream/master` and verify each line is intended. Catch reverts, wrong URLs, unintended file changes before they leave local.
- **Upstream URLs in PR content:** Badges, links, and references in PR-able files must point to upstream repo (trikset/trik-gamepad), never fork (iakov/trik-gamepad).
- **No dead links in README:** Never add a store link unless the app is actually published there. Verify before committing.
- **README audience:** Users first (what it does, screenshots, download, mapping). Developers second (build, lint, test). Remove verbose internals, keep actionable.

### [2026-09-23] PR #41 retrospective — crossed-eye glyph, README restructure, process fixes

**Context:** PR #41 (`fix/readme-restructure`) — crossed-eye glyph for no-video center placeholder synchronised with chip eye, README restructure, dead code cleanup, 3 clean commits. Full gate + CI green.

**What went wrong & how to prevent:**

1. **`git add -A` committed dev-only files into PR branch (twice).** `.omo/`, `.playwright-mcp/`, and stale screenshots leaked into commits. **Fix:** Never use `git add -A` on a PR branch. Always use explicit paths. Pre-commit hook to reject these files would prevent the class.

2. **Fixing the mistake with `git revert` instead of `git reset --hard`.** Creating a "Revert" commit pollutes the history instead of removing the bad commit. **Correct workflow:** `git reset --hard <clean-hash>` + force-push. Never use revert for local history mistakes.

3. **Wrote retrospective to the PR branch as LEARNINGS.md.** Agent-specific session learnings belong in `.omo/` or on the `dev` branch in MEMORY.md, never in the codebase. **Fix:** content boundaries: DECISIONS.md = design decisions (repo, all developers), MEMORY.md = agent memories (dev branch, not upstream), .omo/ = session transient (local only).

4. **Jumped to implementation without classifying intent.** User wanted a glyph, I started analyzing pixels/Unicode/screenshots. **Fix:** Classify intent before any tool call — implementation vs research vs evaluation.

5. **Test assertion assumed wrong final state.** Stream error → UNAVAILABLE → VISIBLE, but retry controller immediately reloads → LOADING → GONE. The test expected VISIBLE when the actual final state is GONE. **Fix:** Trace the full call graph through all collaborators before writing assertions.

6. **Test passed with broken code.** `firstFrameShouldHideVideoPlaceholder` default visibility was already GONE — test wouldn't fail if `updateVideoPlaceholder(PLAYING)` was deleted. **Fix:** Always establish a prior differing state before asserting a transition.

7. **`gh pr edit --title` silently failed.** The command ran without error but title didn't change. **Fix:** Verify with `gh pr view --json title,body` after every PR edit. Use `gh api repos/X/Y/pulls/N -X PATCH` as the reliable path.

8. **Multiple force-pushes with stale PR metadata.** Pushed new commits but didn't update PR title/description. **Fix:** After every push, update PR body via `gh api`. First line of body must match PR title.

9. **Coverage gate: stale jacoco report from build cache.** `jacocoTestCoverageVerification` read a cached report from before test changes. Clean local run passed, CI failed. **Fix:** When coverage fails only on CI, check for cached report first. Add `--no-build-cache` for jacoco tasks or invalidate cache.

**What saved time (keep doing):**

1. **`setVideoStatus()` central helper extracted early.** Eliminated the drift class in one move instead of patching five call sites individually. Rule: when the same two calls appear together in ≥3 places, extract a helper.

2. **`codegraph_explore` before reading files.** One call returned 32 symbols with call paths — replaced dozens of Read/Grep round-trips.

3. **Oracle review caught 7 real issues.** The background oracle found `setVideoLoading` desync, weak test, broken KDoc reference, and duplicated glyph codepoint. Rule: always run an independent Oracle review on non-trivial PRs.

4. **Byte-to-byte diff verification before force-push history rewrite.** Captured diff before rebase, compared after, only pushed when identical. Rule: always capture and verify before rewriting published history.

5. **Pipe output to `.tmp/<step>.log | tail -5`** for post-failure analysis instead of re-running commands.

**Good findings worth persisting:**

- `VideoSourceChip.NONE.glyph` is the single source of truth for the crossed-eye PUA codepoint `\uDB80\uDE09` (U+F0209, md-eye_off in symbols_mono.ttf).
- Using bundled font PUA-B codepoints for custom glyphs is the right approach — renders reliably on every Android device.
- Robolectric runs at 3 SDK levels (`[23]` = minSdk 23 suffix); tests passing at one level but failing at another is usually a Robolectric shadow issue, not a real device problem.
- jscpd has a zero-tolerance (0.0%) duplication gate in CI. Any repeated test setup blocks the build — extract helpers early.
- `runOnUiThread` + retry controller interaction: UNAVAILABLE is set, then `onStreamError()` → `reload()` → LOADING overwrites before the looper drains. Final state after stream error is LOADING, not UNAVAILABLE. Tests MUST account for this.

### [2026-09-23] F-Droid readiness — keystore-free builds, reproducible-build policy

**The keystore path test trap (cost ~20 min):** testing "builds without keystore" by
`mv ../../android-keystorage.p12` from the REPO ROOT moves the WRONG file. The
keystore lives at `<repo-root>/../android-keystorage.p12` (one level above the
repo, relative to `app/`). `file('../../android-keystorage.p12')` in
`app/build.gradle` resolves from the `app/` dir = `<repo-root>/../android-keystorage.p12`.
Always `find /path -name "android-keystorage.p12"` and move THAT file. Second
trap: AGP caches the signing config in the configuration cache — a keystore-free
test must `rm -rf .gradle/configuration-cache` (or `--no-configuration-cache
--rerun-tasks`) or the first keystore-present build pollutes later runs.

**AGP 9 signing-config validation:** `signingConfigs.config` with a `storeFile`
that does NOT exist is only validated when the config is ASSIGNED. Guard the
assignment (`if (keystoreFile.exists()) { signingConfig = ... }` in BOTH
`defaultConfig` and `buildTypes.release`), and the keystore-free build
succeeds (produces an unsigned APK — exactly what F-Droid needs). Using
`storePassword`/`keyPassword` from env vars with `?: ""` fallback keeps the
config valid at configuration time.

**F-Droid release checklist (v2.44 onward):**
0. `uv run python scripts/version_manager.py bump <minor>` (version bump via script ONLY — syncs version.properties + fastlane yml), then `version_manager.py check` must pass.
1. `uv run python scripts/check_reproducibility.py` — zero-timestamp builds + keystore-free `assembleRelease` verified by building twice and comparing unsigned APK hashes.
2. `android.hardware.wifi` declared `required="true"` in the manifest (added 2026-09-23).
3. fastlane metadata complete for en-US/ru/de/fr/vi (title + short/full description).
4. Tag `v2.44` on the release commit; fdroiddata yml references `commit: v2.44`, `gradle: [assembleRelease]`.
5. `android.injected.build.time.zero = true` in `gradle.properties` (zero build timestamps).

**`.omo/` cleanup:** the only git-tracked `.omo/` file was
`store-review-results.md` (from commit 8d694cb) — its content lives on in the
`store-review` skill's tables; deleted from dev. `.omo/run-continuation/*`,
`boulder.json`, and plan scaffolds are pure session transient — deletable at
session end. Session retrospectives belong in MEMORY.md (dev branch), never in
`.omo/` or PR branches.
